...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # creating a 20,000 sat transaction without change, with a manual input, should still be possible
        ...
        # creating a 20,000 sat transaction without change should still be possible
        ...
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00020000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00155090})
        assert_equal("psbt" in res, True)
        assert_equal(res["fee"], Decimal("0.00017060"))

        # creating a 20,000 sat transaction with a manual change address should be possible
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00020000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00010, "changeAddress": addr.pop()})
        assert_equal("psbt" in res, True)
