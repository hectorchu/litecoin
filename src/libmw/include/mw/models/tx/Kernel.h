...
public:
    ...
    Kernel(
        ...
        std::optional<int32_t> lockHeight,
        std::optional<PublicKey> stealthExcess,
    ...
    void Unserialize(Stream& s)
    {
        ...
        if (m_features & STEALTH_EXCESS_FEATURE_BIT) {
            ...
            s >> stealth_excess;
            m_stealthExcess = std::make_optional(std::move(stealth_excess));
        }
...
private:
    uint8_t m_features;
    std::optional<CAmount> m_fee;
    std::optional<CAmount> m_pegin;
    std::vector<PegOutCoin> m_pegouts;
    std::optional<int32_t> m_lockHeight;
    std::optional<PublicKey> m_stealthExcess;
