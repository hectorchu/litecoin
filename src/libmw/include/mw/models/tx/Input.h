...
public:
    ...
    const std::optional<PublicKey>& GetInputPubKey() const noexcept { return m_inputPubKey; }
    ...
    IMPL_SERIALIZABLE(Input, obj)
    {
        ...
        if (obj.m_features & STEALTH_KEY_FEATURE_BIT) {
            ...
            SER_READ(obj, obj.m_inputPubKey = std::make_optional<PublicKey>(std::move(input_pubkey)));
        }
...
private:
    ...
    // The input pubkey.
    std::optional<PublicKey> m_inputPubKey;
