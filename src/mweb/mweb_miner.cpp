...
void Miner::NewBlock(const ChainstateManager& chainman, const uint64_t nHeight)
{
    mweb_builder = std::make_shared<mw::BlockBuilder>(nHeight, chainman.ActiveChainstate().CoinsTip().GetMWEBView());
    hogex_fees = 0;
    hogex_sigops = 0;
...
void Miner::AddHogExTransaction(const CBlockIndex* pIndexPrev, CBlock* pblock, node::CBlockTemplate* pblocktemplate, CAmount& nFees)
{
    ...
    //
    if (!pIndexPrev->hogex_hash.IsNull()) {
