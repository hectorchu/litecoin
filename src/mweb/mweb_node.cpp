...
bool Node::ContextualCheckBlock(const CBlock& block, const Consensus::Params& consensus_params, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, BlockValidationState& state)
{
    // If MWEB is enabled, we must have an mweb_block. If it's not enabled, we must not have an MWEB block.
    if (!DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_MWEB)) {
    ...
    const bool is_first_hogex = !DeploymentActiveAt(*pindexPrev, chainman, Consensus::DEPLOYMENT_MWEB);
...
bool Node::ConnectBlock(const CBlock& block, const Consensus::Params& consensus_params, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, CBlockUndo& blockundo, mw::CoinsViewCache& mweb_view, BlockValidationState& state)
{
    if (!block.mweb_block.IsNull()) {
        const CTransactionRef& pHogEx = block.vtx.back();
