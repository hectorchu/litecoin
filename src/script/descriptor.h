...
private:
    ...
    std::optional<SecretKey> m_scan_key{std::nullopt};
    std::optional<PublicKey> m_spend_pubkey{std::nullopt};
    std::unordered_map<uint32_t, StealthAddress> m_addresses{};
...
public:
    ...
    /** Cache MWEB master scan key
     *
     * @param[in] scan_key The master scan key to cache
     */
    void CacheMWEBMasterScanKey(const SecretKey& scan_key);

    /** Cache MWEB master spend pubkey
     *
     * @param[in] spend_pubkey The master spend public key to cache
     */
    void CacheMWEBMasterSpendPubKey(const PublicKey& spend_pubkey);

    /** Cache MWEB addresss
     *
     * @param[in] mweb_index The index of the address generated by the master keys
     * @param[in] address The address to cache
     */
    void CacheMWEBAddress(const uint32_t mweb_index, const StealthAddress& address);

    bool GetCachedMWEBAddress(const uint32_t mweb_index, StealthAddress& address) const;
    ...
    /** Retrieve cached MWEB scan secret key */
    const std::optional<SecretKey> GetCachedMWEBScanKey() const { return m_scan_key; }
    /** Retrieve cached MWEB spend public key */
    const std::optional<PublicKey> GetCachedMWEBSpendPubKey() const { return m_spend_pubkey; }
    /** Retrieve all cached MWEB addresses */
    const std::unordered_map<uint32_t, StealthAddress> GetCachedMWEBAddresses() const { return m_addresses; }
...
struct Descriptor {
    ...
    /** Whether this is an MWEB descriptor. */
