...
bool CCoinsView::HaveCoin(const GenericOutputID& output_id) const
{
    if (output_id.IsMWEB()) {
        ...
        Coin coin;
        return GetCoin(output_id.ToOutPoint(), coin);
    }
}
...
bool CCoinsViewBacked::GetMWEBCoin(const mw::Hash& output_id, mw::UTXO::CPtr& coin) const { return base->GetMWEBCoin(output_id, coin); }
...
bool CCoinsViewCache::HaveCoin(const GenericOutputID& output_id) const {
    if (output_id.IsMWEB()) {
        auto mweb_cache = GetMWEBCacheView();
        if (mweb_cache) {
            if (mweb_cache->HasCoinInCache(output_id.ToMWEB())) {
                return true;
            }

            if (mweb_cache->HasSpendInCache(output_id.ToMWEB())) {
                return false;
            }
        }
        ...
        CCoinsMap::const_iterator it = FetchCoin(output_id.ToOutPoint());
        return (it != cacheCoins.end() && !it->second.coin.IsSpent());
    }
}

bool CCoinsViewCache::GetMWEBCoin(const mw::Hash& output_id, mw::UTXO::CPtr& coin) const {
    auto mweb_cache = GetMWEBCacheView();
    if (mweb_cache == nullptr) {
        return false;
    }

    if (mweb_cache->HasCoinInCache(output_id)) {
        coin = mweb_cache->GetUTXO(output_id);
        assert(coin != nullptr);
        return true;
    }

    if (mweb_cache->HasSpendInCache(output_id)) {
        return false;
    }
...
bool CCoinsViewCache::HaveCoinInCache(const GenericOutputID& output_id) const {
    if (output_id.IsOutPoint()) {
        CCoinsMap::const_iterator it = cacheCoins.find(output_id.ToOutPoint());
        return (it != cacheCoins.end() && !it->second.coin.IsSpent());
    }
